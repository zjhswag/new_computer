...
def plot_cat_decision_boundary_mc(ax, X, predict , class_labels=None, legend=False, vector=True):
    ...
    ax.contour(xx, yy, Z, linewidths=1) # 绘制分割线，通过0,1,2,3的分割线
    #ax.axis('tight')
...
def plt_mc_data(ax, X, y, classes,  class_labels=None, map=plt.cm.Paired, 
    ...
    for i in range(classes):
        idx = np.where(y == i)  # 返回y==i的所有索引
        col = len(idx[0])*[i]   # len(idx[0]) 得到的是索引长度
...
def plt_cat_mc(X_train, y_train, model, classes):
    ...
    model_predict = lambda Xl: np.argmax(model.predict(Xl),axis=1)  # 返回每行最大值
...
def plt_prob_z(ax,fwb, x0_rng=(-8,8), x1_rng=(-5,4)):
    ...
    tmp_x0,tmp_x1 = np.meshgrid(x0_space,x1_space) # (3,4) #(4,3)
    # print(tmp_x0.shape)
    ...
    for i in range(tmp_x0.shape[0]):
        for j in range(tmp_x1.shape[1]):
            ...
            z[i,j] = fwb(x)                                              # fwb为类似relu函数
            c[i,j] = 0. if z[i,j] == 0 else 1.                           # 绘制大于0和小于0的分割线
...
def plt_output_layer_linear(X, Y, W, b, classes, x0_rng=None, x1_rng=None):
    ...
    for i,axi in enumerate(ax.flat):
        ...
        layerf = lambda x: np.maximum(0, (np.dot(x, W[:, i]) + b[i]))
